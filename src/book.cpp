...
/* Adapted from http://hgm.nubati.net/book_format.html */
...
white king   11
*/
const int kind_of_piece[12] = {p, P, n, N, b, B, r, R, q, Q, k, K};
...
*/
// In Lishex:  enum { WK = 0b0001, WQ = 0b0010, BK = 0b0100, BQ = 0b1000 };
// In Polyglot, the relevant index into the entry list is:
enum {PolyWK = 768, PolyWQ = 769, PolyBK = 770, PolyBQ = 771 };

/* In Polyglot:
"If the opponent has performed a double pawn push and there is now a pawn next
to it belonging to the player to move then "enpassant" is the entry from
RandomEnPassant whose offset is the file of the pushed pawn (counted from
0(=a) to 7(=h)). If this does not apply then enpassant=0."
*/
const int ep_offset = 768;

const int turn_offset = 780;

// offset_piece = 64 * kind_of_piece + 8 * row + file
const uint64_t rand64poly[781] = {
...
uint64_t generate_poly_key(const board_t* board) {
    ...
    for (piece_t p : pieces) {
        ...
        while (b) {
            // offset_piece = 64 * kind_of_piece + 8 * row + file
            key ^= rand64poly[SQUARE_NO * kind_of_piece[p] + POPLSB(b)];
        }
    }

    // Hash in castling rights
    if (board->castle_rights & WK) key ^= rand64poly[PolyWK];
    if (board->castle_rights & WQ) key ^= rand64poly[PolyWQ];
    if (board->castle_rights & BK) key ^= rand64poly[PolyBK];
    if (board->castle_rights & BQ) key ^= rand64poly[PolyBQ];

    // Hash in the en passant square (if set)
    if (board->ep_square != NO_SQ) {
        // In Polyglot, we only hash in the ep square hash if
        // there is a pawn that can capture the double pushed pawn
        // (regardless of whether such capture would be legal)

        // Square with the pawn that can be en passant captured
        square_t pawn_sq = board->ep_square - (board->turn ? NORTH : SOUTH);

        // Piece type of the attacking pawn (either black or white)
        piece_t attacker = set_colour(PAWN, board->turn);

        // If pawn on pawn_sq can be captured, hash in the key
        if (board->pieces[pawn_sq + EAST] == attacker)
            key ^= rand64poly[ep_offset + SQUARE_FILE(pawn_sq)];
        else if (board->pieces[pawn_sq + WEST] == attacker)
            key ^= rand64poly[ep_offset + SQUARE_FILE(pawn_sq)];
    }

    // Hash in the side playing
    if (board->turn == WHITE) {
        key ^= rand64poly[turn_offset];
    }
